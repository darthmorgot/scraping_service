...
class UserUpdateForm(forms.Form):
    city = forms.ModelChoiceField(queryset=City.objects.all(), to_field_name='slug', required=True, label='Город',
                                  widget=forms.Select(attrs={'class': 'form-control'}))
    language = forms.ModelChoiceField(queryset=Language.objects.all(), to_field_name='slug', required=True,
                                      label='Язык программирования',
                                      widget=forms.Select(attrs={'class': 'form-control'}))
    send_email = forms.BooleanField(label='Получать рассылку?', required=False,
...
class ContactForm(forms.Form):
    ...
    language = forms.CharField(required=True, label='Язык программирования',
